...
fn attach_tcp_connects_to_loopback_and_authenticates_first() {
    ...
    let (stream, _) = loop {
        ...
        if let Some(status) = child.try_wait().expect("inspect crow-term") {
            panic!("crow-term exited before connecting: {status}");
        }
        assert!(
            Instant::now() < deadline,
            "crow-term did not connect in time"
        );
