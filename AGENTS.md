...
- Demos: `crow-term --demo` stays on built-in `default` theme; `crow-term --demo-skin` is the gallery (`ember`) path. `make tui-test` drives a dsh profile against the debug bin; `make real-agent-e2e` consumes model tokens — opt-in.
